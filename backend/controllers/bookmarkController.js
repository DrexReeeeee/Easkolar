...
exports.removeBookmark = async (req, res) => {
  const user_id = req.user.user_id;
  const scholarship_id = parseInt(req.params.scholarship_id, 10);
  
  try {
    if (!scholarship_id || isNaN(scholarship_id)) {
      return res.status(400).json({ message: 'Invalid scholarship_id' });
    }
